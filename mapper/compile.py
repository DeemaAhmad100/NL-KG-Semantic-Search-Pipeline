...
def compile_to_cypher(shape: ShapeId, slots: dict) -> tuple[str, dict]:
    ...
    Example:
      ...
      -> ("MATCH (r:Recipe)-[:USES_INGREDIENT]->(:Ingredient {name: $ingredient}) ...",
          {"ingredient": "ginger"})
    """
    # Look up the static template from CANONICAL_CYPHER
    template = CANONICAL_CYPHER[shape]
    
    # Return the template unchanged and the slots dict as params
    # The Neo4j driver will bind $params at query time
